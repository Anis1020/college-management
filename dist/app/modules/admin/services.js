...
const updateAdminIntoDB = (id, payload) => __awaiter(void 0, void 0, void 0, function* () {
    const { name, guardian, localGuardian } = payload, remainingAdminData = __rest(payload, ["name", "guardian", "localGuardian"]);
    const modifiedAdminData = Object.assign({}, remainingAdminData);
    if (name && Object.keys(name).length) {
        for (const [key, value] of Object.entries(name)) {
            modifiedAdminData[`name.${key}`] = value;
        }
    }
    const result = yield schemaModel_1.AdminModel.findByIdAndUpdate(id, payload);
    return result;
});
